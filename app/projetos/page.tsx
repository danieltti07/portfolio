        ...
        <div ref={containerRef} className="min-h-screen pt-16 sm:pt-24 pb-8 sm:pb-16 px-2 sm:px-4 relative overflow-hidden">
            ...
            <div className="absolute inset-0 overflow-hidden pointer-events-none">
                {[...Array(8)].map((_, i) => (
                    <motion.div
                        key={i}
                        className="absolute text-gold/10 font-mono text-xs sm:text-sm"
                        style={{
            ...
            <motion.div style={{ y, opacity }} className="max-w-7xl mx-auto relative z-10">
                ...
                >
                    ...
                    <p className="japanese-text text-cream/90 text-base sm:text-xl max-w-4xl mx-auto leading-relaxed px-4">
                        Uma coleção de projetos que representam minha jornada como desenvolvedor. Cada um conta uma história única
                        de desafios superados, tecnologias dominadas e soluções inovadoras criadas com paixão e dedicação.
                    </p>
                    ...
                    >
                        ...
                        <div className="text-center">
                            <div className="pixel-text text-xl sm:text-3xl text-gold mb-1 sm:mb-2">
                                {projects
                                    .reduce((acc, p) => acc + Number.parseInt(p.metrics.lines.replace(/\D/g, "")), 0)
                                    .toLocaleString()}
                                +
                            </div>
                    ...
                    <div className="flex flex-wrap justify-center gap-2 sm:gap-3">
                        {categories.map((category) => (
                            <motion.button
                                ...
                                whileTap={{ scale: 0.95 }}
                                className={`px-3 sm:px-6 py-2 sm:py-3 rounded-xl pixel-text text-xs sm:text-sm transition-all duration-300 border-2 ${filter === category
                    ...
                    <div className="flex items-center justify-center gap-3">
                        ...
                        <div className="bg-deep-purple/30 backdrop-blur-sm border-2 border-gold/30 rounded-lg p-1">
                            <button
                                ...
                                className={`px-3 sm:px-4 py-2 rounded pixel-text text-xs transition-all ${viewMode === "grid" ? "bg-gold text-deep-purple" : "text-gold hover:bg-gold/10"
                                    }`}
                            >
                                Grade
                            </button>
                            <button
                                ...
                                className={`px-3 sm:px-4 py-2 rounded pixel-text text-xs transition-all ${viewMode === "list" ? "bg-gold text-deep-purple" : "text-gold hover:bg-gold/10"
                                    }`}
                            >
                                Lista
                            </button>
                    ...
                    <div className="relative bg-deep-purple/40 backdrop-blur-sm border-2 border-gold/30 rounded-3xl p-6 sm:p-12 overflow-hidden hover:border-gold transition-all duration-500">
                        ...
                        <div className="relative z-10 text-center">
                            ...
                            <p className="japanese-text text-cream/90 text-base sm:text-xl max-w-3xl mx-auto leading-relaxed mb-6 sm:mb-10">
                                Cada projeto é uma oportunidade de transformar ideias em realidade digital. Vamos colaborar para criar
                                algo extraordinário que faça a diferença no mundo da tecnologia.
                            </p>
...
// =================================================================================
// A MÁGICA ACONTECE AQUI: ProjectCard com o efeito 3D integrado
// =================================================================================
...
function ProjectCard({
    ...
    // --- LÓGICA DO 3D INSERIDA AQUI ---
    const [isHovered, setIsHovered] = useState(false);
    const [mousePosition, setMousePosition] = useState({ x: 0, y: 0 });
    const [mouseOnCardPosition, setMouseOnCardPosition] = useState({ x: 0, y: 0 }); // Para o brilho
    ...
    // Cálculos para a transformação
    const rotateX = isHovered ? (mousePosition.y / 20) * -1 : 0; // Intensidade reduzida
    const rotateY = isHovered ? mousePosition.x / 20 : 0; // Intensidade reduzida
    const scale = isHovered ? 1.05 : 1;
    // --- FIM DA LÓGICA 3D ---


    // MODO LISTA (Layout original mantido)
    if (viewMode === "list") {
        return (
            <motion.div
                ...
                onClick={onSelect}
            >
                {/* ... conteúdo do card de lista sem alteração ... */}
    ...
    // MODO GRID (Layout original + efeito 3D)
    return (
        <motion.div
            initial={{ opacity: 0, y: 50 }}
            animate={{ opacity: 1, y: 0 }}
            ...
            style={{ perspective: "1000px" }} // Adiciona perspectiva para o efeito 3D
            onClick={onSelect}
        ...
        >
            ...
            >
                ...
                {/* Layout original do card de grid */}
...
// O componente ProjectModal permanece exatamente o mesmo do seu código original
    ...
    return (
        ...
        >
            <motion.div
                ...
                className="relative w-full max-w-6xl max-h-[95vh] sm:max-h-[90vh] overflow-y-auto bg-deep-purple border-2 border-gold rounded-2xl sm:rounded-3xl scrollbar-thin scrollbar-thumb-gold/50 scrollbar-track-deep-purple/50"
            >
                {/* ... conteúdo do modal sem alteração ... */}
                <div className={`sticky top-0 z-10 bg-gradient-to-r ${project.color} p-1 rounded-t-2xl sm:rounded-t-3xl`}>
                    <div className="bg-deep-purple/95 backdrop-blur-sm p-4 sm:p-8">
                ...
                <div className="p-4 sm:p-8">
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 sm:gap-12">
                        <div>
                            <div className="relative h-48 sm:h-80 rounded-2xl overflow-hidden mb-6 sm:mb-8 border border-gold/20">
                                <Image src={project.image || "/placeholder.svg"} alt={project.title} fill className="object-cover" />
                            ...
                            <div className="mb-6 sm:mb-8">
                                ...
                                <p className="japanese-text text-cream/90 text-sm sm:text-lg leading-relaxed">
